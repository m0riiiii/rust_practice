...
use crate::routing::{Ipv4Network, AdjRibOut, RibEntry};
use crate::path_attribute::{PathAttribute, AsPath, Origin, self};
...
impl UpdateMessage {
    pub fn new(
        ...
        let path_attributes_length =
            path_attributes.iter().map(|p| p.bytes_len())
            .sum::<usize>() as u16;
        let network_layer_reachability_information_length =
            network_layer_reachability_information.iter().map(|r| r.bytes_len())
            .sum::<usize>() as u16;
        let withdrawn_routes_length =
            withdrawn_routes.iter().map(|w| w.bytes_len())
            .sum::<usize>() as u16;
...
impl From<UpdateMessage> for BytesMut {
    fn from(message: UpdateMessage) -> Self {
        ...
        message
            .path_attributes
            .iter()
            .for_each(|p| bytes.put::<BytesMut>(p.into()));
        message
            .network_layer_reachability_information
            .iter()
            .for_each(|r| bytes.put::<BytesMut>(r.into()));
...
impl TryFrom<BytesMut> for UpdateMessage {
    ...
    fn try_from(bytes: BytesMut) -> Result<Self, Self::Error> {
        ...
        let withdrawn_routes_length: u16 =
            u16::from_be_bytes(bytes[19..21].try_into().context(
                format!("cannot convert from Bytes: {:?} to withdrawn_routes_length", &bytes)
            )?);
        ...
        let total_path_attribute_length = 
            u16::from_be_bytes(bytes[withdrawn_routes_end_index..path_attributes_start_index]
                .try_into().context(format!("cannot convert from Bytes: {:?} to total_path_attribute_length", &bytes))?);
...
mod tests {
    ...
    async fn update_message_from_adj_rib_out() {
        ...
        let local_as: AutonomousSystemNumber = 64514.into();
        let local_ip: Ipv4Addr = "10.0.100.4".parse().unwrap();
